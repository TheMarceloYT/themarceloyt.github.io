...
export class Router {
	...
	static markActiveView(viewSelected) {
		...
		Utils.handleClasesMuchosHTML("remove", `#${this.viewsMenuID}`, ["active"], ".nav-link");

		// dejamos activo el btn del views menu seleccionado
		const btnSelected = viewsMenu.querySelector(`[${Config.nameDataView}="${viewSelected}"]`);
	...
	static async routesHandler(route) {
		...
		if (route === "projects") {
			await Utils.exeFunctions("view", () => this.projectsControllerObj.index());
			this.markActiveView(route);
			return;
		}
		if (route === "contact") {
