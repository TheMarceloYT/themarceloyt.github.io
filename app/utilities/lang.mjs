...
export class Lang {
   ...
   */
  static checkLenguajeApp = (idioma) =>
    Config.lenguajesSoportados.includes(idioma);
  ...
  static markActiveLang(langSelected) {
    ...
    Utils.handleClasesMuchosHTML(
      "remove",
      `#${this.dropdownIdiomaId}`,
      ["active"],
      `[${Config.nameDataLangSelected}]`
    );

    // dejamos activo el btn del idioma seleccionado
    const btnSelected = dropdownIdioma.querySelector(
      `[${Config.nameDataLangSelected}="${langSelected}"]`
    );
  ...
  static updateLenguajeApp() {
    ...
    Utils.doDocumentQueryAll(`[${nombreDataLang}]`).forEach((element) => {
      ...
      element.textContent =
        DataBase.translationTextos[this.getLenguajeActualApp()]?.[clave];
    });
  }
  ...
  static detectIdioma() {
    ...
    if (idiomaGuardado && this.checkLenguajeApp(idiomaGuardado))
  ...
  static listenerLangToggle() {
    document
      .getElementById(Lang.dropdownIdiomaId)
      .addEventListener("click", function (e) {
        ...
        if (btnIdioma) {
          ...
          const idiomaSelected = btnIdioma.getAttribute(
            Config.nameDataLangSelected
          );
          ...
          const nuevoIdioma = btnIdioma.getAttribute(
            Config.nameDataLangSelected
          );
