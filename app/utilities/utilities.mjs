...
export class Utils {
	 ...
	 */
	static getElementPorID = (elementID) => document.getElementById(`${elementID}`);
	 ...
	 */
	static setStorageItem = (nombreItem, item) => localStorage.setItem(nombreItem, item);
	...
	static watchTextChange(query, callback) {
		...
		if (!element) {
			console.log(`[Utils] Elemento HTML NO encontrado para observar: "${query}"`);
			return;
		}
