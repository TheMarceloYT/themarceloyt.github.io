...
export class Theme {
  ...
  static markActiveTheme(themeSelected) {
    ...
    Utils.handleClasesMuchosHTML(
      ...
      `#${this.dropdownTemaId}`,
      ["active"],
      `[${Config.nameDataTheme}]`
    );

    // dejamos activo el btn del tema seleccionado
    const btnSelected = dropdownTheme.querySelector(
      `[${Config.nameDataTheme}="${themeSelected}"]`
    );
  ...
  static getTemaPreferido() {
    ...
    if (temaGuardadoActual && !valoresEspeciales.includes(temaGuardadoActual))
      return temaGuardadoActual;
  ...
  static initTheme() {
    ...
    document
      .getElementById(Theme.dropdownTemaId)
