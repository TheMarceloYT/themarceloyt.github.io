...
export class MYTstrap {
	...
	static initMainOffcanvas() {
		// abrir offcanvas
		document.querySelectorAll(`[data-bs-toggle="${this.mainOffcanvasClase}"]`).forEach((btn) => {
			btn.addEventListener("click", () => {
				...
				if (offcanvas) {
					...
					// muestro backdrop
					const backdrop = this.createDivEnBody(this.MainOffcanvasBackdropClase);
		...
		document.addEventListener("click", function (e) {
			...
			if (toggler) {
				...
				if (dropdown.classList.contains("open")) {
					...
					if (rect.left < 0) {
						dropdownMenu.style.left = `${dropdownMenu.offsetLeft - rect.left}px`;
					}

					// si se sale por abajo (para offcanvas vertical)
					if (rect.bottom > window.innerHeight) {
						const overflowBottom = rect.bottom - window.innerHeight;
						dropdownMenu.style.top = `${dropdownMenu.offsetTop - overflowBottom}px`;
					}
				}
	...
	static initTooltips() {
		...
		document.querySelectorAll(`[${MYTstrap.nameDataTooltipTarget}]`).forEach((el) => {
			...
			el.addEventListener("mouseenter", function (e) {
				...
				const textEl = document.querySelector(`[${MYTstrap.nameDataTooltip}="${target}"]`);
				...
				// no existe
				else console.log(`[MYTstrap] No existe el texto para el tooltip de ${[target]}`);
			});
	...
	static doLoading(tipo, element, id) {
		// existe el elemento?
		if (!element) {
			console.log(`[MYTstrap] No se puede hacer un loader a un elemento inexistente`);
			return;
		}
